...
AlarmState currentAlarmState = AlarmState::Normal; // Initialize the current alarm state to Normal
...
void PinModeSetup()
{
    ...
    pinMode(_WaterPin, INPUT_PULLUP);   // Set Water pin as input with
...
void loop()
{
    ...
    if (SilenceFlag && !TimeOut)
    {
        ...
        States[(uint8_t)currentAlarmState](1); // Not sure if this casts the current state into an uint8_t or fetch's the equivalent
    }
}
